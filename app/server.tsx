...
const getData = cache(async () => {
  ...
  const { data: trusteds } = await supabase.from('trusted').select('name, image');

  const { data: features } = await supabase.from('feature').select('title, description, icon');
  ...
  const { data: comments } = await supabase.from('comment').select('title, description, image');
