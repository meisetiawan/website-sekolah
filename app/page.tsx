...
const api = cache(async () => {
  ...
  const { data: faqs, error: faqError } = await supabase.from('faq').select('question, answer');
  if (faqError) throw new Error(faqError.message);

  const { data: sliders, error: sliderError } = await supabase.from('slider').select('title, image');
  if (sliderError) throw new Error(sliderError.message);
  ...
  return { about, trusteds, histories, features, section, comments, faqs, sliders, credit };
});

async function AsyncSection(properties: any) {
  const { about, trusteds, histories, features, section, comments, faqs, sliders, credit } = await api();
  return (
    <>
      <Client
        supabase={{ about, trusteds, histories, features, section, comments, faqs, sliders, credit }}
      />
    </>
  );
}
...
export default function App() {
  return (
    <>
      ...
      <MoonDefinition />
      <div className="relative flex min-h-screen w-full flex-col items-center justify-center overflow-hidden bg-[#020712]">
